...
rln.question('What is your name? ', (name) => {
...
var nameGreeting = nameCheck('Vasya');
...
const bestMovie = (movie_title) => `${movie_title} is the best movie ever!`
...
function theEliminator (cts, lsr) {
  for (let cont of cts) {
    if (cont = lsr) {
      cts.splice(cts.indexOf(cont), 1)
    }
  }
  return cts;
}
...
const chocoFrogs = (gold) => (parseInt(gold) - parseInt(gold) % 3) / 3;
var totalFrogs = chocoFrogs(100500);
...
// I did it the right way in the initial task.

const chocoFrogs1 = (gold) => (parseInt(gold) - parseInt(gold) % 3) / 3;
var totalFrogs1 = chocoFrogs1(4);
...
function fOo (array) {
  if (array.every(function (arg, item) {return item === 0 || arg >= array[item - 1]})) {
    return true;
  } else {
    return false;
  }
